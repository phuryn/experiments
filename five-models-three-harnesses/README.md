...
> **A sixth arm was added on July 16–17: Kimi K3 (2.8T, open weights), in [kimi-k3-day-one/](../kimi-k3-day-one/).** Two findings below do not survive it. **"Zero false positives from any arm"** — K3 posts 2, the first in this rig. **Opus's planted-bug crown (12/21)** — K3 caught 14. K3 borrowed Claude Code's harness (it has no CLI of its own) and ran on launch-day infrastructure, so it is not an equal test surface: one of its eight cells never finished at all, and the same cell that returned zero tokens on launch night scored 14/14 thirteen hours later. This folder stays as-is, as the July 10 record.
