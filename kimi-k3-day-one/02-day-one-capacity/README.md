...
4. **Fixing the shim was not enough.** With 200-wrapped throttles retried, the cell got far past its old death point (9-28 turns became a 67-minute run) and still timed out. The pool is simply too hot. **The stale-docs cell stays DNF** — a capacity result, honestly reported, not a model score. *(Update, Jul 18: it stayed DNF only as long as the pool stayed hot — the identical config ran clean on day 3 with zero throttle events and scored. See [03-day-three-recovery](../03-day-three-recovery/).)*
...
- `kimi_proxy2.py` also sets `--reasoning-effort max`, matching Moonshot's documented contract (`max` is the only supported level; "do not use the K2.x thinking parameter"). The other K3 cells ran without that flag. At the time of this experiment the stale-docs cell had produced no score, so the flag had affected no published number. *(As of Jul 18 it has — the day-3 completion in [03](../03-day-three-recovery/) is scored, and the disclosure is a headline caveat there.)*
