# Recursive workflow depth test — spec (set 05, per-run dir: @DIR@)
...
3. Otherwise, WRITE a new bash script at @DIR@/gen_L<N+1>.sh (you are authoring the next level's orchestrator) that does exactly:
   ...
   - runs: claude -p "Read @DIR@/gen_spec.md and execute the procedure with LEVEL=<N+1>" --model sonnet --allowedTools "Bash,Write,Read" --permission-mode acceptEdits < /dev/null
