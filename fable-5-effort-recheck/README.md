...
- **Reroute check (added same day, post-publish).** Fable requests can silently reroute to another model under availability pressure, so the response metadata was audited: **146/147 fable rows report `claude-fable-5`; zero rerouted.** All 66 opus rows report `claude-opus-4-8`. The one fable row without a model field is a killed timeout with no response payload (`needle-fable-max-r03`, rc −9, ~700s).
