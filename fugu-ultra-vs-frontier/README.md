...
The short answer: it is reliable, it catches no more cross-file bugs than GPT-5.5, and it costs ~3x more per run while a hidden orchestration tax inflates its real token bill ~12x over what you see in the reply.
...
| 3 | Cost per run | Cheapest tokens win? | **GLM $0.10, GPT-5.5 $0.49, Opus $0.51, fugu ~$1.48.** fugu is the most expensive model in the panel per run. |
...
**Headline result (the tweet):** *fugu-ultra is reliable but the worst value here — it catches no more cross-file bugs than GPT-5.5 while costing ~3x per run, and the conductor's hidden ~12x token tax buys no diligence edge.*
...
Exact tokens from each model's in-context audit run × list price (input billed at the in rate, visible+orchestration output at the out rate). Means over n=10:
...
| fugu-ultra | **~$1.48** | $1.50 | Sakana list, in $5 / out $30, all billed tokens |

fugu-ultra is the most expensive model in the panel per run — ~3x GPT-5.5 and Opus, ~15x GLM — driven by the orchestration tax, not by a higher per-token price. Total fugu spend across this set was ~$45. Cost rows: [planted_bug_incontext.csv](planted_bug_incontext.csv), [gpt55_planted_incontext.csv](gpt55_planted_incontext.csv), [opus_planted_incontext.csv](opus_planted_incontext.csv), [glm_planted_incontext.csv](glm_planted_incontext.csv).
...
3. **fugu-ultra is the worst value in the panel.** ~$1.48/run vs GPT-5.5's $0.49 and Opus's $0.51 — ~3x the price for the same diligence (4/10 vs GPT-5.5's 5/10, a tie within noise) and the same reliability (100%). The conductor architecture buys no measurable edge here over a single frontier model.
...
- **Costs are list-price estimates**, computed from the exact token counts in the CSVs at each vendor's published rates — not from invoices. No prompt-cache discount is applied (the in-context audits send a fresh ~30K–130K-token corpus each run). The fugu figure averages over the 10 in-context runs, two of which truncated short; mean ~$1.48, median $1.50.
